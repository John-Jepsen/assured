    ...
    async def run(
        ...
        *,
        ...
        if session.pending_payment:
            # Only a *bare* yes/no answers the pending "shall I confirm this payment?".
            # Such a reply carries no other intent, so route it to billing to complete or
            # cancel the charge. Anything else means the caller moved on without answering
            # — the pending charge expires so a stray "yes" on a later, unrelated turn can
            # never complete it, and the current turn routes normally (preserving any other
            # intent it carried, e.g. a human-escalation request).
            if reply_polarity(message) is not None:
