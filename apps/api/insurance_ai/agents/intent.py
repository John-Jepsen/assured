...
_INTENT_RULES: dict[str, tuple[AgentName, list[str]]] = {
    "policy_info": (AgentName.POLICY, [
        r"\bpolic(y|ies)\b", r"coverage", r"\bcover(s|ed|ing)?\b", r"\bdeductible", r"\blimit",
        r"\bexclusion", r"\brenew", r"\beffective date", r"\bvehicle\b", r"\brental car",
        r"\bchange .*(vehicle|car|policy)", r"\badd .*(driver|vehicle)", r"\bbeneficiar",
    ]),
    "claim": (AgentName.CLAIMS, [
        r"\bclaim", r"\bfile a\b", r"\baccident", r"\badjuster", r"\bfnol",
        r"\bfirst notice", r"\bdamage\b", r"\bloss\b", r"\bdispute",
    ]),
    "billing": (AgentName.BILLING, [
        r"\bbill", r"\bpay(ment|ing)?\b", r"\bpremium", r"\bbalance", r"\bautopay",
        r"\binvoice", r"\bcharge", r"\bpast due", r"\bmake a payment", r"\bnext payment",
        r"\bincrease", r"\bwent up", r"\bcost more",
    ]),
    "account": (AgentName.ACCOUNT, [
        r"\bmy (account|profile|policies|info)", r"\bcontact info", r"\bupdate my (email|phone|address)",
        r"\bcommunication preference", r"\bwhat policies",
    ]),
    "scheduling": (AgentName.SCHEDULING, [
        r"\bschedule", r"\bappointment", r"\bcall me", r"\bcall with", r"\bbook a",
        r"\breschedule", r"\bcancel .*(appointment|call)",
    ]),
    "escalation": (AgentName.ESCALATION, [
        r"\bhuman\b", r"\bagent\b(?!.*call)", r"\brepresentative", r"\bspeak to (a|someone)",
        r"\bmanager", r"\bcomplaint", r"\bthis is ridiculous", r"\bunacceptable",
    ]),
    "general": (AgentName.GENERAL, [
        r"\bwhat is\b", r"\bwhat does\b", r"\bhow do i\b", r"\bexplain\b", r"\bmean(s|ing)?\b",
        r"\bhelp\b", r"\bfaq\b", r"\bterminolog", r"\bdifference between",
    ]),
}
...
def extract_entities(text: str) -> Entities:
    ...
    for m in _POLICY_RE.finditer(text):
        val = m.group(1).upper()
        if val.startswith("CLAIM-") or val.startswith("INV-"):
            continue
...
_PRODUCT_HINTS: dict[str, list[str]] = {
    ...
    "health": [r"\bhealth\b", r"\bmedical\b", r"\bcopay", r"\bcoinsurance", r"\bdeductible.*health"],
...
def detect(text: str) -> IntentResult:
    ...
    if not scores:
        return IntentResult(
            intents=["general"], agents=[AgentName.GENERAL], scores={},
            entities=extract_entities(text), ambiguous=True,
        )
    ...
    has_account_context = bool(
        entities.policy_numbers or entities.claim_numbers or entities.invoice_numbers
    ...
    return IntentResult(
        intents=selected, agents=agents, scores=scores,
        entities=extract_entities(text), ambiguous=ambiguous,
    )
