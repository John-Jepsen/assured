...
async def conversations(db: AsyncSession = Depends(get_session), limit: int = 50) -> dict:
    ...
    return {"conversations": [
        {"id": c.id, "channel": c.channel, "verification_status": c.verification_status,
         "current_agent": c.current_agent, "escalated": c.escalated,
         "customer_id": c.customer_id, "updated_at": c.updated_at.isoformat()}
        for c in rows.scalars().all()
    ]}


@router.get("/conversations/{conversation_id}")
async def conversation_detail(conversation_id: str, db: AsyncSession = Depends(get_session)) -> dict:
    ...
    return {
        "conversation": {
            "id": conv.id, "channel": conv.channel,
            "verification_status": conv.verification_status,
            "current_agent": conv.current_agent, "escalated": conv.escalated,
        },
        "transcript": transcript,
        "tool_executions": [
            {"tool_name": t.tool_name, "ok": t.ok, "error_code": t.error_code,
             "arguments": t.arguments, "latency_ms": round(t.latency_ms, 1),
...
async def customer_profile(customer_id: str, db: AsyncSession = Depends(get_session)) -> dict:
    ...
    return {
        ...
        "zip_code": c.zip_code, "synthetic": c.is_synthetic,
        "policies": [{"policy_number": p.policy_number, "product_type": p.product_type,
                      "status": p.status} for p in policies.scalars().all()],
    }
...
async def tickets(db: AsyncSession = Depends(get_session)) -> dict:
    ...
    return {"tickets": [
        {"ticket_number": t.ticket_number, "status": t.status, "urgency": t.urgency,
         "reason": t.reason, "summary": t.summary, "handoff": t.handoff,
...
async def tool_activity(db: AsyncSession = Depends(get_session), limit: int = 100) -> dict:
    ...
    return {"tool_executions": [
        {"tool_name": t.tool_name, "ok": t.ok, "error_code": t.error_code,
         "latency_ms": round(t.latency_ms, 1), "conversation_id": t.conversation_id,
         "created_at": t.created_at.isoformat()} for t in items
    ], "avg_latency_ms": round(
        sum(t.latency_ms for t in items) / len(items), 1) if items else 0}


@router.get("/evaluations")
async def evaluations(db: AsyncSession = Depends(get_session)) -> dict:
    rows = await db.execute(select(EvaluationRun).order_by(desc(EvaluationRun.created_at)).limit(20))
    return {"runs": [
        {"suite": r.suite, "total": r.total, "passed": r.passed,
         "pass_rate": round(r.passed / r.total, 3) if r.total else 0,
         "created_at": r.created_at.isoformat()} for r in rows.scalars().all()
    ]}
